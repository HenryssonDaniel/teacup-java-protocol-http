...
import static org.teacup.protocol.http.Constants.VERIFY;
...
class DurationNodeImpl implements DurationNodeSetter {
  private static final Logger LOGGER = Logger.getLogger(DurationNodeImpl.class.getName());

  private Node<Duration> abs;
  private ObjectAssert<? super Duration, ?> assertion;
  private ObjectAssert<Integer, IntegerAssert> nano;
  private Node<Duration> negated;
  ...
  @Override
  public void setAbs(DurationNode abs) {
    LOGGER.log(Level.FINE, String.format(SETTING_THE, "abs"));
    this.abs = abs;
  }

  @Override
  public void setAssertion(ObjectAssert<? super Duration, ?> assertion) {
  ...
  @Override
  public void setNegated(DurationNode negated) {
  ...
  private static void verifyDuration(Node<? super Duration> node, Duration duration) {
